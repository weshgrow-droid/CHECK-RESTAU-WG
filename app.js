...
function renderTickets(byClient, checks, today) {
  ...
  for (const client of clients) {
    ...
    head.innerHTML = `<span class="client">${escapeHtml(client)}</span><span class="count">${lines.length} référence${lines.length > 1 ? "s" : ""}</span>`;
    ...
    for (const l of lines) {
      ...
      line.className = "line";
      line.innerHTML = `<span class="ref mono">${escapeHtml(l.ref)}</span><span class="qty mono">×${escapeHtml(l.qty)}</span>`;
