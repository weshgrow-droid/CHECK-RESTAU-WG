...
function todayDDMMYYYY() {
  return dateToDDMMYYYY(new Date());
}
...
// Date actuellement sélectionnée (DD/MM/YYYY), pilotée par le sélecteur de date
function selectedDateDDMMYYYY() {
  if (!els.datePicker.value) return todayDDMMYYYY();
  const [yyyy, mm, dd] = els.datePicker.value.split("-");
  return `${dd}/${mm}/${yyyy}`;
}
...
window.addEventListener("load", () => {
  els.datePicker.value = dateToYYYYMMDD(new Date());
  els.todayLabel.textContent = selectedDateDDMMYYYY();
...
async function loadOrders() {
  ...
  try {
    ...
    const selectedDate = selectedDateDDMMYYYY();
    const todays = orders.filter((o) => o.date === selectedDate);
    ...
    renderTickets(byClient, checks, selectedDate);
...
function renderTickets(byClient, checks, today) {
  ...
  for (const client of clients) {
    ...
    info.innerHTML = `
      ...
      <div class="stats"><b>${estimatedColis}</b> colis estimé${estimatedColis > 1 ? "s" : ""}</div>
    `;
